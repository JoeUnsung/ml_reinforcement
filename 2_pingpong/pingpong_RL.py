# -*- coding: utf-8 -*-
"""
Created on Fri Dec  8 12:05:48 2017
...
EMPTY = 0
PADDLE_HEIGHT = 360.0
PADDLE_MOVE = [-35, 35, 0]   # PADDLE이 좌우로 이동하는 속도

START = 3
END = 4

class Ball:
    def __init__(self, canvas, paddle, color, announceterm, saveterm, winval = 10, loseval = 0):
        self.canvas = canvas
        self.paddle = paddle
    ...
    def greedyChoice(self):
        ...
        elif self.lookup(val_left) < self.lookup(val_right) and self.lookup(val_right) > self.lookup(val_stop):
            return 1
        
        elif self.lookup(val_stop) > self.lookup(val_left) and self.lookup(val_right) < self.lookup(val_stop):
            return -1
        else:
    ...
    def keystate(self,movement):
        ...
        return (int(paddle_pos[0]/39),(int(ball_pos[0]/39),ball_pos[1]),(self.x,self.y),movement)
...
class Paddle:
    def __init__(self, canvas, y_loc, color):
        ...
        self.canvas.move(self.id, 180, y_loc)  # 패들 사각형을 200,300 에 위치
...
if __name__ == '__main__':
    ...
    canvas = Canvas(tk, width=490, height=450, bd=0, highlightthickness=0)
